...
export const mapDispatchToProps = (dispatch, ownProps) => {
    return {
        ...
        initiate:{
            ...
            /**
             *
             * @param searchVal false时第一次初始化serach的时候，初始数据需要保存在临时对象中，用于页面切换显示时使用
             */
        ...
        applicationForAdmission:{
            ...
            handleOperate: async (record,isSubmit=false,fun) => {
                // 保存/提交
                // dispatch(setBtnLoadingActive());
                // dispatch(setBtnRequestDisplay());
                Global.showLoading();

                let ajaxFun = api.in_hosp_apply_temp;
        ...
        dischargeAssessment:{
            ...
            handleOperate: async (record,isSubmit,fun) => {
                ...
                // dispatch(setBtnRequestDisplay());
                Global.showLoading();
                let ajaxFun = api.out_hosp_apply_temp;
                let ajaxParam = record;
                if(isSubmit){
                    ajaxFun = api.user_commit;
                    ajaxParam = {...record,commitType:Static.commitType.kf}
